...
import { UsuarioUseCase, usuarioUseCase } from "../../core/usuario/useCases/usuario.use-case";
...
class UsuarioController {
  ...
  criarUsuario = async (request: FastifyRequest<CriarUsuarioRoute>, reply: FastifyReply) => {
  ...
  buscarUsuarioPorId = async (request: FastifyRequest<BuscarUsuarioPorIdRoute>, reply: FastifyReply) => {
  ...
  buscarUsuarios = async (request: FastifyRequest<BuscarUsuariosRoute>, reply: FastifyReply) => {
    const { search, inativos, cargo } = request.query;

    const usuarios = await this.usuarioUseCase.buscarUsuarios({ search, inativos, cargo });

    reply.status(HTTP_STATUS.SUCCESS).send(usuarios);
  };

  editarUsuario = async (request: FastifyRequest<EditarUsuarioRoute>, reply: FastifyReply) => {
    const { id } = request.params;
